...
/**
* \file grille.h
* \brief Fonctions permettant de gerer la grille, sa sauvegarde, son affichage et structures de données.
...
/**
* \struct bloc_s
* \brief bloc de défense créer par une unité.
*/
...
/**
* \struct case_s
* \brief represente une case du terrain avec sa position, pour pouvoir l'afficher.
*/
...
/**
 * \fn void initialisation_principale(int bordure,SDL_Window * pWindow,int * largeur, int * hauteur,joueurs_t tab[J],degatx_t aff_deg[AFF_DEG],bash_t tab_info_bash[TAILLE_TAB_BASH],case_t terrain[N][M],char variable2[80])
...
/**
 * \fn void affichage_principale(SDL_Renderer *renderer,SDL_Window* pWindow,int bordure,int * largeur,int * hauteur,joueurs_t tab[J],degatx_t aff_deg[AFF_DEG],bash_t tab_info_bash[TAILLE_TAB_BASH],case_t terrain[N][M],int joueur_actu,image_t image[Z],int compteur_anim,int nb_joueur_restant,int compteur_tour,int frame_anim_montre,int jour_nuit,char variable2[80],int compteur_bouton_cam)
 * \brief affiche le terrain
*/
/**
 * \fn void carte_valide(case_t terrain[N][M])void carte_valide(case_t terrain[N][M])
 * \brief corrige la map pour qu'elle ne possède pas de zone inaccessible.
*/
/**
 * \fn int reste_ennemi(case_t terrain[N][M],int joueur_actu)
 * \brief retourne le nb d'ennemi restant en jeu
*/
/**
 * \fn int reste_allie(case_t terrain[N][M],int joueur_actu)
 * \brief retourne le nb d'alliés restant en jeu
*/
/**
 * \fn int allie_adjacent(case_t terrain[N][M],int x_bot,int y_bot,int joueur_actu)
 * \brief revoie 1 si un allié est directement dans une des 4 cases adjacentes
*/
/**
 * \fn void update_grille(case_t terrain[N][M],int compteur_tour,bash_t tab_info_bash[TAILLE_TAB_BASH],char variable2[80])
