 ...
 * \brief remplis la grille de jeu (terrain[x][y].attaque= ?) selon si la case est apte a l'attaque d'un allié
 */
/**
 * \fn int calc_block(case_t terrain [N][M],int x_att, int y_att, int x_def,int y_def);
 * \brief calcul les chances de bloquer une attaque selon la position de l'adversaire
 */
/**
 * \fn void combat(case_t terrain [N][M],int x_att, int y_att, int x_def,int y_def,  int joueur,joueurs_t tab[J],degatx_t aff_deg[AFF_DEG],  bash_t tab_info_bash[TAILLE_TAB_BASH],char variable2[80]);
 * \brief calcul les dommages infligés a une unités lors d'un combat
 */
/**
 * \fn void update_stats(case_t terrain[N][M],int x,int y,int joueur_actu,joueurs_t tab[J])
 * \brief amélioration des stats apres une élimination en fonction de la classe.
 */
